...
plt.xlim((0,12.5))
plt.plot(qs, 3*np.sqrt(-scp.special.jv(1,qs)*qs), c = 'r')     # 2D Fermi
#plt.ylim((0,20))
plt.xlabel(r"$k a$")
plt.ylabel(r"$t \omega_{coll}$")
plt.colorbar(sc)
plt.tight_layout()
...
plt.xlim((0,15))
plt.plot(qs, np.sqrt(-scp.special.jv(1,qs)*qs), c = 'r')     # 2D Fermi#plt.ylim((0,6))
plt.xlabel(r"$k_x$[$\omega_{pe} c^{-1}$]")
